...
module Services
  ...
  class Session
    ...
    def login(credentials:)
      response = self.class.post("/login", obtain_options(body: credentials.to_json))
    ...
    def create_user(user_data:)
      response = self.class.post("/signup", obtain_options(body: user_data.to_json))
      @auth_token = response.parsed_response["token"] if response.success?
      handling_response(response: response)
    end
    ...
    def index_categories
      response = self.class.get("/categories", obtain_options)
      handling_response(response: response)
    end

    # new_category: { name: string, transaction_type: expense }
    def create_category(new_category:)
      response = self.class.post("/categories", obtain_options(body: new_category.to_json))
      handling_response(response: response)
    end

    def show_category(category_id:)
      response = self.class.get("/categories/#{category_id}", obtain_options)
      handling_response(response: response)
    end

    # new_data : { name: string, transaction_type : string }
    def update_category(category_id:, new_data:)
      response = self.class.patch("/categories/#{category_id}", obtain_options(body: new_data.to_json))
      handling_response(response: response)
    end

    def delete_category(category_id:)
      response = self.class.delete("/categories/#{category_id}", obtain_options)
      handling_response(response: response)
    end

    # ------------------------------- TRANSACTIONS Services -------------------------------
    def index_transactions(category_id:)
      response = self.class.get("/categories/#{category_id}/transactions", obtain_options)
      handling_response(response: response)
    end
    ...
    def create_transaction(category_id:, new_data:)
      response = self.class.post("/categories/#{category_id}/transactions",
                                 obtain_options(body: new_data.to_json))
      handling_response(response: response)
    end
    ...
    def update_transaction(category_id, transaction_id, transaction_new_data)
      response = self.class.patch("/categories/#{category_id}/transactions/#{transaction_id}",
                                  obtain_options(body: transaction_new_data.to_json))
      handling_response(response: response)
    end

    def delete_transaction(category_id:, transaction_id:)
      response = self.class.delete("/categories/#{category_id}/transactions/#{transaction_id}", obtain_options)
      handling_response(response: response)
    end
    ...
    # Body -> JSON format
    def obtain_options(body: nil)
      toptions = { headers: obtain_headers }
